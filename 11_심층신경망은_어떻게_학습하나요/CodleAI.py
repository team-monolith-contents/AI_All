...
class AIProject3():
    def __init__(self,a,b,c,d,e):
        self.__init = widgets.FloatSlider(description='초기값 :', min=-300, max=300) # a 변수의 값을 조절할 수 있는 FloatSlider를 만들어줍니다.
        self.__lr = widgets.FloatSlider(description='학습률 :', min=-300, max=300) # b 변수의 값을 조절할 수 있는 FloatSlider를 만들어줍니다.
        self.__out = widgets.Output() # ipywidget의 output을 만들어줍니다.
        # self.__init_value = 0
        # self.__init_lr = 0
        
        self.__x = np.arange(0,100,0.1)
        self.__a = a
        self.__b = b
        self.__c = c
        self.__d = d
        ...
        display(self.__init,self.__lr,self.__out) # a,b,out을 출력단에 표시해줍니다.

        with self.__out:
            self.__fig = plt.figure()
            self.__draw()

        self.__init.observe(self.__on_init_value_change, names='value') # a가 변화할 때 마다 실행 될 콜백함수를 등록합니다.
        self.__lr.observe(self.__on_lr_value_change, names='value') # b가 변화할 때 마다 실행 될 콜백함수를 등록합니다.
    
    def __equation(self,x):
        return self.__a*x**4 + self.__b*x**3 + self.__c*x**2 + self.__d*x + self.__e
    
    # 미분 함수.
    def __derivative(self,x):
        return 4*self.__a*x**3 + 3*self.__b*x**2 + 2*self.__c*x + self.__d

    # ipywidget의 output을 그려주는 함수입니다.
    def __draw(self):
        clear_output(wait=True) # ipywidget에 포함된 함수로 output을 초기화 시켜주는 함수입니다.
        
        plt.plot(self.__x, self.__equation(self.__x))
        plt.plot(self.__init.value,self.__equation(self.__init.value), c = "red")
        print(self.__init.value, self.__equation(self.__init.value))
        # while True:
            

    # init 값이 변경될 때 실행될 콜백함수입니다.
    def __on_init_value_change(self, change):
        with self.__out: # with out: block에 있는 코드들은 출력값을 ipywidget output으로 보내줍니다.
            # self.__init_value = change['new']
            self.__draw()

    # lr 값이 변경될 때 실행될 콜백함수입니다.
    def __on_lr_value_change(self, change):
        with self.__out:
            # self.__lr_value = change['new']
            self.__draw()
